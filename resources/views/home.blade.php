        ...
        <div class="col-lg-6">
            <a href='/profiles'><h2>Nieuwste gebruikers</h2></a>
                ...
                @foreach($profiles as $profile)
                        <a href="/profiles/{{$profile->user_id}}"><img style="vertical-align: middle;" class="img-circle" height=50px width=50px src="{{ route('profile.image', ['filename' => $profile->username . '-' . $profile->user_id . '.jpg']) }}" alt="" class="img-responsive"></a>
                        <h3 style="display: inline-block; padding-left: 20px;"><a href="/profiles/{{$profile->user_id}}">{{$profile->username}}</a></h3>
                        <br>
